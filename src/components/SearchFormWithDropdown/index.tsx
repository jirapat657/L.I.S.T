...
import { statusOptions as staticStatusOptions } from '@/constants/searchFilters';
...
const SearchFormWithDropdown: React.FC<SearchFormProps> = ({
  ...
  // ดึงข้อมูล users สำหรับ dropdown
  ...
  // กำหนด options ต่างๆ
  const statusOptions = staticStatusOptions;
  const developerOptions = getDeveloperOptions(users);
  const baTestOptions = getBATestOptions(users);

  const handleFilterChange = <K extends keyof FilterValues>(
    field: K, 
