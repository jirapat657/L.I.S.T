...
export const useGenerateIssueCode = (projectDocId: string | undefined, form: FormInstance) => {
  ...
  useEffect(() => {
    ...
    console.log(
      '📦 issueCodes ทั้งหมด:',
      issues.map((i) => i.issueCode)
    )

    // กรองเฉพาะ issue ที่ตรงเดือนและปีนี้
    const filtered = issues.filter((issue) => issue.issueCode?.startsWith(prefix))
